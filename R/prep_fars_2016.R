...
prep_fars_2016 <- function(y, wd, rawfiles, prepared_dir, geo_filtered){
...
# person-level files ----
  ...
  ## safetyeq ----
    ...
    fars.safetyeq <-
      ...
      usenames() %>%
      select(-setdiff(intersect(names(fars.person), names(.)), c("state", "st_case", "veh_no", "per_no"))) %>%
      mutate( # Format adjustment to align with y>=2017
        nmhelmet = ifelse(msafeqmt == "Helmet", "Yes", "No"),
        nmpropad = ifelse(msafeqmt == "Protective Pads (elbows, knees, shins, etc.)", "Yes", "No"),
        nmothpro = ifelse(msafeqmt == "Other Safety Equipment", "Yes", "No"),
        nmrefclo = ifelse(msafeqmt == "Reflective Clothing/Carried Item", "Yes", "No"),
        nmlight  = ifelse(msafeqmt == "Lighting", "Yes", "No"),
        nmothpre = ifelse(msafeqmt == "Other Safety Equipment", "Yes", "No")
        ) %>%
      select(-msafeqmt)
