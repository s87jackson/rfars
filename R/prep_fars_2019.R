...
prep_fars_2019 <- function(y, wd, rawfiles, prepared_dir, geo_filtered){
...
# vehicle-level files ----
  ...
  ## multi-row files ----
    ...
    # fars.pvehiclesf <-
    #   fars.vehicle %>%
    #   select(state, st_case, veh_no, pveh_sc1, pveh_sc2) %>%
    #   mutate_all(as.character) %>%
    #   pivot_longer(cols = -c(1:3), values_to = "pvehiclesf") %>%
    #   select(-name) %>%
    #   unique()
    #
    # fars.vehicle <- fars.vehicle %>% select(-starts_with("pveh_sc"))
